<?php
	...
	if (strlen($searchTerm) == 0) {
		$stmt = $dbcon->prepare("SELECT * FROM groups WHERE name LIKE :term AND `closed` = 0 ORDER BY id DESC LIMIT 11 OFFSET :offset;");
	}else{
		$stmt = $dbcon->prepare("SELECT * FROM groups WHERE name LIKE :term AND `closed` = 0 ORDER BY name ASC LIMIT 11 OFFSET :offset;");
	}
	...
	foreach($stmt as $result) {
		...
		if ($count < 11) {
			...
			echo '<h4><a href="/groups/view/'.$result['id'].'">'.context::secureString($result['name']).'</h4></a><b style="display:inline">Description: </b><p style="display:inline">'.$description.'</p>';
			echo '</div></div></div>';
		}
	}
	if ($count > 10) {
