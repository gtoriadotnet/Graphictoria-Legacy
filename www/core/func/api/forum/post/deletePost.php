<?php
	if (isset($_POST['csrf']) and isset($_POST['postId'])) {
		...
		$stmt = $GLOBALS['dbcon']->prepare("SELECT deleted, author_uid, forumId FROM topics WHERE id = :id AND developer = 0 ORDER BY id DESC LIMIT 1;");
		$stmt->bindParam(':id', $postId, PDO::PARAM_INT);
		$stmt->execute();
		$result = $stmt->fetch(PDO::FETCH_ASSOC);
		if ($stmt->rowCount() == 0 || $result['deleted'] === 1) {
			echo 'error';
			exit;
		}
		...
		$query = "UPDATE `topics` SET `deleted`=1 WHERE `id`=:id;";
		$stmt = $GLOBALS['dbcon']->prepare($query);
		$stmt->bindParam(':id', $postId, PDO::PARAM_INT);
		$stmt->execute();
		...
		$query = "SELECT * FROM topics WHERE forumId=:id AND `deleted`=0;";
		$stmt = $GLOBALS['dbcon']->prepare($query);
