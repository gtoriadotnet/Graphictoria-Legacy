<?php
	if (isset($_POST['replyContent']) and isset($_POST['csrf']) and isset($_POST['postId'])) {
		...
		if (strlen($replyContent) < 1 or strlen($contentCheck) < 1) {
		...
		if ($timeSince < 1 and $result['rank'] == 0) {
			echo 'rate-limit';
			exit;
		}
		...
		if($shortenedContent != $replyContent)
		{
		...
		if (($result['locked'] == 1 || $result['deleted'] === 1) and $GLOBALS['userTable']['rank'] == 0) {
			echo 'access-denied';
			exit;
		}
