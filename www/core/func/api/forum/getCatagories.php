<?php
	...
	foreach($stmt as $result) {
		...
		$stmt->execute();
		foreach($stmt as $result) {
			echo '<li><p><a class="forum-clickable" onclick="loadForum('.$result['id'].')">'.context::secureString($result['name']).'</a></p></li>';
		}
		echo '</ul>';
	}
	if($GLOBALS['loggedIn'])
	{
		echo '<h3 style="font-size:18px">Manage</h3><ul><li><p><a class="forum-clickable" onclick="loadForum(\'my-posts\')">Your Topics</a></p></li></ul>';
	}
?>