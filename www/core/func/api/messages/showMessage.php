<?php
	if (isset($_GET['id'])) {
		...
		<a href="/user/profile/'.$userSheet['username'].'"><img height="150" width="150" class="img-responsive" style="display:inline" src="'.($userSheet['id'] == 15 ? 'https://gtoria.net/core/html/img/logo.png' : context::getUserImage($userSheet)).'"></a><br>';
		if ($userSheet['rank'] == 1) {
		...
		if ($userSheet['rank'] > 0) {
			...
			$content = preg_replace("/https?:\/\/[^ ]+?(?:\.jpg|\.jpeg|\.png|\.gif)/",'<br/><img class="img-responsive" src="$0"><br/>', $content);
		}
