	...
	foreach($stmt as $resultOwned) {
		...
		if ($count < 7) {
			...
			if ($result['deleted'] == 0) {
				...
				echo '<img style="max-height:100px;display:inline" class="img-responsive" src="'.context::getItemThumbnailC($type, $result['id'], $result['datafile'], $result['fileHash'], $result['imgTime']).'">';
				if ($wearing == true) {
