<?php
	...
	if (isset($_GET['id'])) {
		...
		$stmt = $dbcon->prepare("SELECT username, id, thumbnailHash, headshotHash FROM users WHERE id = :id");
		$stmt->bindParam(':id', $result['creator_uid'], PDO::PARAM_INT);
		$stmt->execute();
		$resultuser = $stmt->fetch(PDO::FETCH_ASSOC);
		echo '<div class="col-xs-6">';
		if (file_exists($_SERVER["DOCUMENT_ROOT"] . "/../api/imageServer/server/".$result['id'].".png") && $GLOBALS['loggedIn']) {
			echo "<img style=\"max-height:100%;max-width:100%\" src=\"http://api.gtoria.net/imageServer/server/".$result['id'].".png?v=".strtotime($result['imgTime'])."\">";
		}
		echo '</div>';
		echo '<div class="col-xs-6">';
		...
		if ($GLOBALS['loggedIn']) {
			//if ($result['version'] == 3) echo '<a class="btn btn-success fullWidth" href="GraphictoriaClient16://'.$GLOBALS['userTable']['gameKey'].';'.$result['id'].';'.$GLOBALS['userTable']['id'].'">Play</a>';
			if ($result['version'] == 3) echo '<a class="btn btn-success fullWidth" href="GraphictoriaClient16:1+launchmode:play+placelauncherurl:https%3A%2F%2Fapi.gtoria.net%2FGame%2FPlaceLauncher.ashx%3FplaceId%3D' . $result['id'] . '+gameinfo:' . $GLOBALS['userTable']['gameKey'] . '">Play</a>';
		}else{
