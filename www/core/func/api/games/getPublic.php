<?php
	...
	if ($version == 0) {
		...
		$stmt = $GLOBALS['dbcon']->prepare("SELECT * FROM games WHERE public = 1 AND version = 3 ORDER BY id DESC");
	}else{
	...
	function getImage($result2, $serverID, $imgTime) {
		if (file_exists("/var/www/api/imageServer/server/".$serverID.".png") && $GLOBALS['loggedIn']) {
			return "http://api.gtoria.net/imageServer/server/".$serverID.".png?v=".strtotime($imgTime);
		}else{
	...
	function getVersion($gVersion) {
		if ($GLOBALS['gameVersion'] == 4) {
			if ($gVersion == 3) $versionString = "2016";
