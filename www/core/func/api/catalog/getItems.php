	...
	if (isset($_GET['type'])) {
		...
		foreach($stmt as $result) {
			...
			if ($count < 16) {
				...
				echo '<img class="img-responsive" style="max-height:140px;max-width:140px;display:inline;" src="'.context::getItemThumbnailC($type, $result['id'], $result['datafile'], $result['fileHash'], $result['imgTime']).'">';
				echo '</div>';
		...
		if ($count > 15) {
			echo '<button class="btn btn-primary loadMore" onclick="loadMoreItems(\''.$type.'\', ' . ($page+1) . ((isset($term) and strlen($term) > 0) ? ', \'' . urlencode($term) . '\'' : null) . '); page++;">Load More</button>';
		}
		echo '</div>';
	}else{
