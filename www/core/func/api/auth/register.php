<?php
	if (isset($_POST['username']) && isset($_POST['email']) && isset($_POST['passwd1']) && isset($_POST['passwd2']) && isset($_POST['csrf']) && isset($_POST['captcha'])) {
		...
		$response = json_decode(file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=6Le6TFsaAAAAAP4P8qsmFoQ3DXkGx21R7wFSWvVb&response=".$_POST['captcha']."&remoteip=".auth::getIP()), true);
		...
		if(!preg_match("/^[a-zA-Z0-9]+[\w\.]?[a-zA-Z0-9]+$/", $username) == 1 || str_contains($username, "\n") || str_contains($username, "\r") || str_contains($username, "\t")) {
		...
		$good_emails = array('@outlook', '@protonmail.com', '@gtoria.net', '@roblox.com', '@icloud.com', '@protonmail.ch', '@google.com',
		...
		if(context::contains($email, ["@outlook.com", "@outlook.com.br", "@outlook", "@aol.com", "@yahoo."])) die('no-outlook');
