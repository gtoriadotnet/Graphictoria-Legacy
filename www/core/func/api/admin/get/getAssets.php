<?php
	...
	foreach($stmt as $result) {
		...
		echo '<img width="150" src="https://gtoria.net/data/assets/uploads/'.$result['fileHash'].'"><img width="150" style="filter:invert(1)" src="https://gtoria.net/data/assets/uploads/'.$result['fileHash'].'"><br><img width="150" style="filter:contrast(0)" src="https://gtoria.net/data/assets/uploads/'.$result['fileHash'].'"><img width="150" style="filter:contrast(1)" src="https://gtoria.net/data/assets/uploads/'.$result['fileHash'].'"><br><b>Type : '.$result['type'].'</b><br><b>Uploaded by <a href="/user/profile/'.$username.'">'.$username.'</a></b><br><button type="submit" name="acceptAsset" class="btn btn-success" onclick="approveAsset('.$result['id'].');">Accept</button><button type="submit" name="denyAsset" class="btn btn-danger" onclick="denyAsset('.$result['id'].');">Decline</button></div></div>';
	}
?>