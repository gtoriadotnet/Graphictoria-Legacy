	...
	if (isset($_POST['csrf']) and isset($_POST['itemID'])) {
		...
		if ($result['approved'] == 0 and $result['declined'] == 0) {
			if ($type == "shirts" or $type == "pants" or $type == "tshirts") {
				context::requestImage($assetID, $type);
			}
			...
			if ($type != "decals") {
				$message = 'Your asset named '.$assetName.' has been approved and can be seen in the catalog. You also have received the item in your inventory. Your item can be found at https://gtoria.net/catalog/item/'.$itemID;
			}else{
				$message = 'Your asset named '.$assetName.' has been approved and can be seen in the catalog. Your item can be found at https://gtoria.net/catalog/item/'.$itemID;
			}
			
			$title = 'Asset Approval result for '.$assetName;
			$query = "INSERT INTO messages (`recv_uid`, `sender_uid`, `title`, `content`) VALUES (:userId2, 15, :title, :msg);";
			$stmt = $GLOBALS['dbcon']->prepare($query);
