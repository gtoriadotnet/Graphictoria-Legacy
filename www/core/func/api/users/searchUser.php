<?php
	...
	foreach($stmt as $result) {
		...
		if ($count < 11) {
			...
			$usern = '';
			if ($result['banned'] == 1) {
				$usern = '<i class="text-muted">'.$result['username'].'</i>';
			}elseif ($result['rank'] == 0) {
				$usern = $result['username'];
			}elseif ($result['rank'] == 1) {
				$usern = '<b style="color:#158cba">'.$result['username'].'</b>';
			}elseif ($result['rank'] == 2) {
				$usern = '<b style="color:#28b62c">'.$result['username'].'</b>';
			}
			
			echo '<h4>'.context::getOnline($result).' <a href="/user/profile/'.$result['username'].'">'.$usern.'</h4></a><b>Last seen:</b> '.$lastSeen.'<br><b style="display:inline">About: </b><p style="display:inline">'.$about.'</p>';
			echo '</div></div></div>';
		}
	}
	if ($count > 10) {
