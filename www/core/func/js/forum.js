...
{
	// xlxi on 7/2/2021 do not steal plz :D
	$(".adContainer").css("display", "none");
	$("#posts").html('<br><br><br><div class="center"><i class="fa fa-circle-o-notch fa-spin fa-3x fa-fw"></i></div>');
	$("#posts").load("/core/func/api/forum/editPost.php?id=" + postId);
	$("html, body").animate({ scrollTop: 0 }, "fast");
}
...
function postMessage(forumId) {
	if ($("#postMessage").is(":disabled") == false) {
		...
		.done(function(response) {
			...
			}else if (response == "no-newline-spam") {
				$("#pStatus").css("color", "red").html("Hey there! Your post is a little too spammy, calm down a little with the new lines!");
	...
	if ($("#postReply").is(":disabled") == false) {
		...
		.done(function(response) {
			...
			if (response == "error") {
				...
				$("#rStatus").css("color", "red").html("Hey there! Your post is a little too spammy, calm down a little with the new lines!");
			}else if (response == "rate-limit") {
...
function doEditPost(postId)
{
	if ($("#doEditPost").is(":disabled") == false) {
		$("#doEditPost").prop("disabled", true);
		$("#postContent").prop("disabled", true);
		
		var postContent = $("#postContent").val();
		var csrf_token = $('meta[name="csrf-token"]').attr('content');
		$.post('/core/func/api/forum/post/editPost.php', {
			postContent: postContent,
			csrf: csrf_token,
			postId: postId
		})
		.done(function(response) {
			$("#doEditPost").prop("disabled", false);
			$("#postContent").prop("disabled", false);
			
			if (response == "error") {
				$("#rStatus").css("color", "red").html("Network error. Try again later.");
			}else if (response == "content-too-short") {
				$("#rStatus").css("color", "red").html("Your post is too short.");
			}else if (response == "content-too-long") {
				$("#rStatus").css("color", "red").html("Your post is too long.");
			}else if (response == "no-newline-spam") {
				$("#rStatus").css("color", "red").html("Hey there! Your post is a little too spammy, calm down a little with the new lines!");
			}else if (response == "rate-limit") {
				$("#rStatus").css("color", "red").html("You are editing too fast. Please wait.");
			}else if (response == "no-post") {
				$("#rStatus").css("color", "red").html("The post you are trying to edit to does not exist.");
			}else if (response == "access-denied") {
				$("#rStatus").css("color", "red").html("Access Denied.");
			}else{
				$("#rStatus").html(response);
