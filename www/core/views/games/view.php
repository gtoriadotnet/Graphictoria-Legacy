			...
			<div class="col-xs-12 col-sm-4 col-md-6 col-sm-offset-2 col-md-offset-3col-xs-12 col-sm-4 col-md-6 col-sm-offset-2 col-md-offset-3">
				...
				<div class="well profileCard">
					<div class="row">
						...
						<div class="col-md-8 col-sm-12 col-xs-12" style="word-wrap:break-word;">
							<?php
								if ($result['version'] == 3) $vString = "2016";
							?>
				...
				<div class="well profileCard">
					...
					<div class="row center">
						<?php
							$stmt = $dbcon->prepare("SELECT lastSeen, inGame, username, thumbnailHash, headshotHash, id FROM users WHERE inGameId = :id ORDER BY id");
							$stmt->bindParam(':id', $id, PDO::PARAM_INT);
							$stmt->execute();
							$count = 0;
				...
				<?php
					if ($GLOBALS['loggedIn']) {
						if ($GLOBALS['userTable']['id'] == $result['creator_uid'] && $result['dedi'] == 0) {
							echo '<div class="well"><h4>Command</h4><p>Use this command to start your server</p><code>dofile("http://api.gtoria.net/serverscripts/server.php?key='.$result['privatekey'].'")</code></div>';
						}
						if ($result['public'] == 0) {
